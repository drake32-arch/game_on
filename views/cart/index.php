	...
	<div class="table-responsive">
    	<table class="table">
	        ...
	        <?php foreach($cartItems as $item): ?>
	            <tr>
	                <td><?=Html::img("@web/images/{$item->getProduct()->picture}", ['alt' => $item->getProduct()->name, 'width' => 50])?></td>
	                <td><a href="<?=Url::to('@web/products/index?id=' . $item->getProduct()->id_product)?>"><?= $item->getProduct()->name ?></a></td>
			...
			<tr class="btn btn-success">
		            <td colspan="4"><?= Html::a('Оформить заказ', Url::to('@web/offer/checkout')) ?></td>
